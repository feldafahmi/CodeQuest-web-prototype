...
import {
  ...
  CloudRain, Sun, Moon, Menu, Umbrella, Cat, Dog, Minus, Home, ClipboardList,
...
function Btn({ children, variant = "primary", color = C.violet, style, disabled, ...props }) {
  const base = {
    ...
    opacity: disabled ? 0.45 : 1, transition: "all 0.2s ease-in-out", ...style,
  };
  const variants = {
...
function Badge({ children, color = C.violet }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 600, color, background: bg, padding: "4px 10px", borderRadius: 999 }}>
      {children}
    </span>
  );
}
...
export default function App() {
  ...
  return (
    <div className={isDarkMode ? "dark-mode" : "light-mode"} style={{
      ...
      <div style={{ position: "absolute", top: 10, right: 10, zIndex: 110, display: "flex", gap: 6, background: "var(--panel2)", border: `1px solid ${C.panelBorder}`, borderRadius: 999, padding: 4, boxShadow: "0 4px 12px rgba(0, 0, 0, 0.15)" }}>
      ...
      {page === "landing" && <Landing onStart={() => setPage("login")} isDarkMode={isDarkMode} onToggleTheme={() => setIsDarkMode(!isDarkMode)} />}
      {page === "login" && <Login onLogin={(role) => setPage(role)} isDarkMode={isDarkMode} onToggleTheme={() => setIsDarkMode(!isDarkMode)} />}
...
/* ---------------------------------------------------------------
   LANDING
--------------------------------------------------------------- */
function Landing({ onStart, isDarkMode, onToggleTheme }) {
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      <nav className="landing-nav" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "24px 60px" }}>
        <Logo />
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <button onClick={onToggleTheme} style={{
            ...
            display: "inline-flex", alignItems: "center", justifyContent: "center", padding: 8, borderRadius: "50%",
            transition: "all 0.2s ease"
          }}>
      ...
      <div className="hero-layout" style={{ padding: "40px 60px 80px", alignItems: "center", flex: 1 }}>
        <div>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 28, justifyContent: "inherit" }}>
...
/* ---------------------------------------------------------------
   LOGIN / REGISTER
--------------------------------------------------------------- */
function Login({ onLogin, isDarkMode, onToggleTheme }) {
  const [mode, setMode] = useState("login");
  const [role, setRole] = useState("student");

  return (
    <div className="login-layout" style={{ flex: 1 }}>
      <div className="login-left-panel-content" style={{ padding: "48px 64px", display: "flex", flexDirection: "column", position: "relative", flex: 1 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", maxWidth: 440, width: "100%", margin: "auto" }}>
      ...
      <div className="login-right-panel" style={{ background: `radial-gradient(circle at 50% 40%, ${C.violetSoft}, var(--bg-soft) 70%)`, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 18, borderLeft: `1px solid var(--panel-border)` }}>
        <Robot size={140} />
...
function SidebarItem({ icon: Icon, label, active, onClick, color }) {
  return (
    <div onClick={onClick} className={`sidebar-item-btn ${active ? "active" : ""}`} style={{
  ...
  return (
    <div className={`app-sidebar ${className}`}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22 }}>
        <Logo size={16} />
        <button className="sidebar-close-btn" onClick={onCloseSidebar} style={{
          display: "none", background: "transparent", border: "none", cursor: "pointer", padding: 4
        }}>
          <X size={18} />
        </button>
      </div>
...
function TopBar({ right, isDarkMode, onToggleTheme, onToggleSidebar }) {
  return (
    <div style={{ display: "flex", alignItems: "center", padding: "16px 24px", borderBottom: `1px solid ${C.panelBorder}`, gap: 14 }}>
      ...
      {sorted.map((s, i) => (
        <div key={s.name} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 8px", borderRadius: 12, background: i === 0 ? C.violetSoft : "transparent", marginBottom: 4 }}>
          <div style={{
            ...
            background: i < 3 ? C.amber : "var(--rank-bg)", color: i < 3 ? "#0A0D18" : C.muted,
          }}>{i + 1}</div>
  ...
  return (
    <div className="tutor-layout">
        ...
        <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 12, paddingRight: 6 }}>
          {messages.map((m, i) => (
            <div key={i} style={{ display: "flex", justifyContent: m.from === "user" ? "flex-end" : "flex-start" }}>
              <div style={{ maxWidth: "70%", padding: "10px 14px", borderRadius: 14, fontSize: 13.5, lineHeight: 1.5, background: m.from === "user" ? C.violet : "var(--panel2)", color: m.from === "user" ? "#0A0D18" : C.text }}>{m.text}</div>
            </div>
          ))}
        </div>
        <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
          <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()}
            placeholder="Tulis pertanyaanmu..." style={{ flex: 1, background: "var(--panel2)", border: `1px solid ${C.panelBorder}`, borderRadius: 12, padding: "10px 14px", color: C.text, fontSize: 13 }} />
      ...
      <Card>
        ...
        {topics.map((t) => (
          <div key={t} onClick={() => setInput(`Jelaskan tentang ${t}`)} style={{ fontSize: 13, color: C.muted, padding: "8px 4px", cursor: "pointer", transition: "color 0.2s" }} onMouseEnter={(e) => e.currentTarget.style.color = C.violet} onMouseLeave={(e) => e.currentTarget.style.color = C.muted}>{t}</div>
        ))}
      </Card>
    </div>
  );
  ...
  return (
    <Card>
      ...
      <div className="responsive-grid-3">
        {badges.map((b) => (
          <div key={b.name} style={{ border: `1px solid ${C.panelBorder}`, borderRadius: 14, padding: 16, textAlign: "center", opacity: b.earned ? 1 : 0.45, background: "var(--panel)", transition: "all 0.2s ease-in-out" }} className="hover-lift">
  ...
  return (
    <div className="responsive-grid-2">
      <Card>
        ...
        <input defaultValue={role === "student" ? "Alex Putra" : "Andi Wijaya"} style={{ width: "100%", background: "var(--panel2)", border: `1px solid ${C.panelBorder}`, borderRadius: 10, padding: "10px 12px", color: C.text, fontSize: 13, margin: "6px 0 14px" }} />
        <label style={{ fontSize: 12, color: C.muted }}>Email</label>
        <input defaultValue={role === "student" ? "alex.putra@email.com" : "andi.wijaya@sekolah.sch.id"} style={{ width: "100%", background: "var(--panel2)", border: `1px solid ${C.panelBorder}`, borderRadius: 10, padding: "10px 12px", color: C.text, fontSize: 13, margin: "6px 0 14px" }} />
        <label style={{ fontSize: 12, color: C.muted }}>{role === "student" ? "Sekolah" : "Mata Pelajaran"}</label>
        <input defaultValue={role === "student" ? "SMA Negeri 1" : "Informatika"} style={{ width: "100%", background: "var(--panel2)", border: `1px solid ${C.panelBorder}`, borderRadius: 10, padding: "10px 12px", color: C.text, fontSize: 13, margin: "6px 0 18px" }} />
...
/* ---------------------------------------------------------------
   WORLD PAGE (materi + soal per level)
--------------------------------------------------------------- */
    ...
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        <button onClick={() => setView("dashboard")} style={{
          ...
          background: "var(--panel2)", border: `1px solid ${C.panelBorder}`, color: C.text,
          cursor: "pointer", fontWeight: 600, fontSize: 13, padding: "8px 16px",
          borderRadius: 10, transition: "all 0.2s ease-in-out"
            ...
            return (
              <div key={l.name} onClick={() => selectLevel(i)} style={{
                ...
                <div style={{
                  ...
                  background: done ? world.color : "var(--btn-inactive-bg)",
    ...
    <Card>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {q.options.map((opt, i) => {
          ...
          return (
            <div key={i} onClick={() => choose(i)} className={isRight ? "correct-bounce" : ""} style={{
      ...
      {selected !== null && (
        <div style={{ marginTop: 14, padding: "10px 14px", borderRadius: 10, background: "var(--panel2)", fontSize: 13, color: C.muted }}>
          💡 {q.explain}
        </div>
      )}
  ...
  return (
    <Card>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 18, maxWidth: 300 }}>
        {["🤖", "", "🌳", "", "🏠"].map((e, i) => (
          <div key={i} style={{ flex: 1, height: 42, borderRadius: 8, background: "var(--panel2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>{e}</div>
        ))}
      </div>
      ...
      {ran && <div className="correct-bounce" style={{ marginTop: 16, padding: "10px 14px", borderRadius: 10, background: `${world.color}1F`, color: world.color, fontSize: 13, fontWeight: 600 }}>🎉 Benar! Robot berhasil mencapai rumah.</div>}
    ...
    <Card>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
        {Array.from({ length: total }).map((_, i) => (
          <div key={i} style={{
            ...
            background: i < collected ? `${world.color}22` : "var(--panel2)", opacity: i < collected ? 0.4 : 1,
      ...
      <div style={{ background: "var(--panel2)", borderRadius: 12, padding: 14, marginBottom: 18 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: `${world.color}22`, color: world.color, borderRadius: 8, padding: "8px 12px", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600, fontSize: 13, width: "fit-content" }}>
          repeat
          <button onClick={() => { setCount((c) => Math.max(1, c - 1)); setRan(false); }} style={{ background: "rgba(0,0,0,0.2)", border: "none", borderRadius: 6, width: 20, height: 20, color: world.color, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><Minus size={12} /></button>
          <span>{count}</span>
          <button onClick={() => { setCount((c) => Math.min(total, c + 1)); setRan(false); }} style={{ background: "rgba(0,0,0,0.2)", border: "none", borderRadius: 6, width: 20, height: 20, color: world.color, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><Plus size={12} /></button>
          times
        </div>
    ...
    <Card>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {["JavaScript", "Python", "Scratch"].map((l) => (
            <button key={l} onClick={() => switchLang(l)} style={{ fontSize: 12, padding: "6px 12px", borderRadius: 999, cursor: "pointer", border: `1px solid ${lang === l ? color : C.panelBorder}`, background: lang === l ? `${color}22` : "transparent", color: lang === l ? color : C.muted, transition: "all 0.2s" }}>{l}</button>
          ))}
        </div>
      </div>
      <div className="playground-layout">
        <textarea
          ...
          spellCheck={false}
          style={{
            background: "var(--panel2)", borderRadius: 12, padding: 14, fontFamily: "monospace", fontSize: 13, lineHeight: 1.7,
...
function TeacherArea({ view, setView, unlocked, isDarkMode, onToggleTheme }) {
  ...
  return (
    <div style={{ display: "flex", flex: 1, position: "relative" }}>
      ...
      <div style={{ flex: 1, minHeight: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <TopBar isDarkMode={isDarkMode} onToggleTheme={onToggleTheme} onToggleSidebar={() => setSidebarOpen(true)} right={<Btn onClick={() => setShowModal(true)}><Plus size={14} /> Buat Kelas</Btn>} />
        <div style={{ flex: 1, padding: "22px 26px", overflowY: "auto" }}>
      ...
      {STUDENTS.map((s) => (
        <div key={s.name} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr", alignItems: "center", padding: "12px 8px", fontSize: 13, borderBottom: `1px solid ${C.panelBorder}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 28, height: 28, borderRadius: "50%", background: "var(--panel2)", display: "flex", alignItems: "center", justifyContent: "center" }}><Bot size={14} color={C.muted} /></div>
            {s.name}
          </div>
          <div>{s.level}</div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {LEVELS[w.id].map((l, i) => {
            ...
            return (
              <div key={l.name} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", alignItems: "center", padding: "9px 4px", fontSize: 13, borderBottom: i < LEVELS[w.id].length - 1 ? `1px solid ${C.panelBorder}` : "none" }}>
                <div>{l.name}</div>
                <div><Badge color={l.type === "interactive" ? w.color : C.mutedSoft}>{l.type === "interactive" ? "Misi" : "Quiz"}</Badge></div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
  ...
  return (
    <div style={{ position: "absolute", inset: 0, background: "var(--modal-overlay)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 120 }}>
      <Card style={{ width: 360, position: "relative" }}>
        ...
        <input placeholder="Nama kelas (contoh: XI IPA 2)" style={{ width: "100%", background: "var(--panel2)", border: `1px solid ${C.panelBorder}`, borderRadius: 10, padding: "10px 12px", color: C.text, fontSize: 13, marginBottom: 16 }} />
